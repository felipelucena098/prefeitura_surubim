...
//RECUPERAR DADOS DO USUÁRIO
$query = $pdo->query("SELECT * FROM dados_pessoais where id = '$_SESSION[id_usuario]' and limitar_cadastro = '1'");
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$limitar_cadastro = @$res[0]['limitar_cadastro'];
...
$query = $pdo->query("SELECT * FROM dados_pessoais where limitar_cadastro = '1' and id_usuario = '$_SESSION[id_usuario]' ");	
$res = $query->fetchAll(PDO::FETCH_ASSOC);
$total_reg = @count($res);
	...
	if($id == "" && $total_reg == 0){
	$res = $pdo->prepare("INSERT INTO dados_pessoais SET id_usuario = :id_usuario, nome = :nome, cpf = :cpf, rg = :rg, orgao_exp = :orgao_exp, nasc = :nasc, nis = :nis, rua = :rua, bairro = :bairro, cep = :cep, email = :email, telefone = :telefone, uf = :uf, limitar_cadastro = '1', documentos = :documentos");
	...
	$res = $pdo->prepare("UPDATE dados_pessoais SET id_usuario = :id_usuario, nome = :nome, cpf = :cpf, rg = :rg, orgao_exp = :orgao_exp, nasc = :nasc, nis = :nis, rua = :rua, bairro = :bairro, cep = :cep, email = :email, telefone = :telefone, uf = :uf, documentos = :documentos WHERE id = '$id'");
